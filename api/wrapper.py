...
class Wrapper:
	...
	def getLists(self):
		results, metadata = cypher.execute(self.graphdb, "MATCH (u:User)-[]->(l:List) WHERE u.username = {username} RETURN l", params = [["username", self.username]]);
	...
	def getItemsFromList(self, listId):
		results, metadata = cypher.execute(self.graphdb, "MATCH (l:List)-[]->(i:Item) WHERE id(l) = {listId} RETURN i ", params = [["listId", listId]]);
